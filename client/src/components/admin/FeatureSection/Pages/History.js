...
const History = () => {
  ...
  const outcomesArr = () => {
    axios.get("http://localhost:3001/api/get_outcomes").then((response) => {
      const countAdmitted = response.data.filter(
        (obj) => obj.outcome_id === 1
      ).length;
      const countNoShow = response.data.filter(
        (obj) => obj.outcome_id === 2
      ).length;
      const countUserCancelled = response.data.filter(
        (obj) => obj.outcome_id === 3
      ).length;
      const countAdminCancelled = response.data.filter(
        (obj) => obj.outcome_id === 4
      ).length;
      const countWaiting = response.data.filter(
        (obj) => obj.outcome_id === 5
      ).length;
